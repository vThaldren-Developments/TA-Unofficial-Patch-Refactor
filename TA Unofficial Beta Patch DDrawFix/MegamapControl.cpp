...
//PatchFix::Patching::Patch SkipGameRender1;
//
//__declspec(naked) void SkipGameRender1Func()
//{
//	__asm
//	{
//		jmp [SkipGameRender1.ReturnAddr]
//	}
//}
	...
	int MaxIconWidth, int MaxIconHeight, int MegaMapVirtulKey_arg, BOOL WheelMoveMegaMap_v, BOOL DoubleClickMoveMegamap_v, BOOL WheelZoom_v)
{
	...
	//SkipGameRender1.Setup(PATCHTYPE_INTERCEPT, null, null, JUMPTYPE_JMP, (VoidPtr)0x00469624, SkipGameRender1Func, (VoidPtr)0x00469F23);
...
BOOL MegaMapControl::SelectUp (int x, int y, bool out, bool shift)
{
	...
	if (selectbuttom::select==SelectState)
	{
			...
			&&((SelectTick/ 1000)<= (GetTickCount ( )/ 1000)))
		{
			...
			myExternHotKey->SelectUnitInRect ( ALL, &SelectScreenRect, shift);
			SelectedCount= myExternHotKey->FilterSelectedUnit ( );
			//
			UpdateSelectUnitEffect ();
			ApplySelectUnitMenu_Wapper ( );
