...
BOOL TABugFixing::AntiCheat (void)
{
	...
	if (TRUE==*IsCheating)
	{
		(*TAmainStruct_PtrPtr)->SoftwareDebugMode = (softwaredebugmode::SOFTWAREDEBUGMODE)((*TAmainStruct_PtrPtr)->SoftwareDebugMode| softwaredebugmode::Now);
	}
	else
	{
		(*TAmainStruct_PtrPtr)->SoftwareDebugMode= (softwaredebugmode::SOFTWAREDEBUGMODE)(((*TAmainStruct_PtrPtr)->SoftwareDebugMode)& (~softwaredebugmode::Now));
...
BOOL Gb_TempEHA= false;
DWORD  (WINAPI resume_EHA) (LPVOID)
{
	Sleep (5000);
	Gb_TempEHA= false;
	return TRUE;
...
bool TABugFixing::Message(HWND WinProcWnd, UINT Msg, WPARAM wParam, LPARAM lParam)
{
	if (Msg==WM_ACTIVATE )
	{
		//Gb_TempEHA= true;
		//DWORD dwTID;
		//CreateThread ( NULL, 0, resume_EHA,0 ,0, &dwTID);
		//resumeScreenPlatte();
		//resumeOffScreen ();
		//((IDDraw *)(LocalShare->DDrawClass))->resume();
	}
	return false;
}
