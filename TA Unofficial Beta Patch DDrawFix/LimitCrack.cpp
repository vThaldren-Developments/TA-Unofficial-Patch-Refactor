...
/// I had put anything into this file, so, the file name isn't that important at now :\

//int tick_AIFrame() 0464F80
...
IncreaseAISearchMapEntriesLimit::IncreaseAISearchMapEntriesLimit ()
{
	...
	m_NewLimit = 66650;
	ModifyTheLimit= new SingleHook ( reinterpret_cast<LPBYTE>(AISearchMapEntriesLimit), 4, INLINE_UNPROTECTEVINMENT, reinterpret_cast<LPBYTE>(&m_NewLimit));


	//BYTE _5NOP[5] = { 0x90, 0x90, 0x90, 0x90, 0x90 };
	//avoidOrginalTick = new SingleHook(AddrOrginalPathFind, 5, INLINE_UNPROTECTEVINMENT, _5NOP);

	//pathFindThd= CreateThread ( NULL, 0, AI_PathFindThd, this, 0, &pathFindTID);
...
{
	m_Work_b = FALSE;
	//WaitForSingleObject ( pathFindThd, 1000);
	delete ModifyTheLimit;

	//delete avoidOrginalTick;
}
