...
ExternHotKey::ExternHotKey ()
{
	...
	//IDDrawSurface::OutptTxt ( "New ExternHotKey");
}
...
bool ExternHotKey::Message(HWND WinProcWnd, UINT Msg, WPARAM wParam, LPARAM lParam)
{
...
// 			}
		...
		case WM_KEYDOWN:
			...
			//if (0x53==(int)wParam)
			//{//ctrl+s
				if (wParam == 0x53 && (GetAsyncKeyState(VK_CONTROL)&0x8000)>0 && (GetAsyncKeyState(VK_SHIFT)&0x8000)==0)
				{
...
void ExternHotKey::FindIdelFactory ()
{
...
void ExternHotKey::FindIdleConst()
{
	//FixAck();
	//
...
int ExternHotKey::SelectOnlyInScreenSameTypeUnit (BOOL FirstSelect_Flag)
...
int ExternHotKey::SelectOnlyInScreenWeaponUnit (unsigned int SelectWay_Mask)
{
...
int ExternHotKey::SelectUnitInRect (TAUnitType NeededType, RECT * rect, bool shift)
{
	if (NULL==rect)
	{
		return 0;
...
int ExternHotKey::FilterSelectedUnit (void)
{
...
int ExternHotKey::FilterSelectedUnitProc (TAUnitType NeededType) //只会在已选中的单位中选择!!!!速度很慢，全部单位都枚举一遍!
{
...
int ExternHotKey::InitExternTypeMask (void)
{
	...
	for (int i= 0; i<TypeCount; ++i)
	{
		Current= &Begin[i];
		if ((0!=(builder&Current->UnitTypeMask_0))
			&&(NULL!=Current->bmcode)//able to move
			&&(! MatchInTypeAry ( Current->UnitTypeID, CommanderMask))
			)
		{
	...
	for (int i= 0; i<TypeCount; ++i)
	{
		...
		if ((builder&Current->UnitTypeMask_0)
			//&&(! (canmove&Current->UnitTypeMask_0)) // dont select mobile
			&&(NULL==Current->bmcode)
			&&(! MatchInTypeAry ( Current->UnitTypeID, CommanderMask)))
		{
...
void ExternHotKey::DestroyExternTypeMask (void)
{
	for	(int i= 0; i<RACENUMBER; ++i)
	{
...
int __stdcall AddtionRoutine_CircleSelect (PInlineX86StackBuffer X86StrackBuffer)
{
	...
	__try	
	{
		...
		if (TAInGame==DataShare->TAProgress)
		{
			ExternHotKey * this_myExternHotKey= (ExternHotKey *)(X86StrackBuffer->myInlineHookClass_Pish->ParamOfHook);
			int Temp=  this_myExternHotKey->FilterSelectedUnit ( );
...
{
	ExternHotKey * this_myExternHotKey= (ExternHotKey *)(X86StrackBuffer->myInlineHookClass_Pish->ParamOfHook);
	this_myExternHotKey->InitExternTypeMask ( );
