...
CMapRect::CMapRect(BOOL VidMem)
{
	...
	MapX = &(*TAmainStruct_PtrPtr )->EyeBallMapXPos;
	MapY = &(*TAmainStruct_PtrPtr )->EyeBallMapYPos;

	MapSizeX = &(*TAmainStruct_PtrPtr)->MapSizeX;
	MapSizeY = &(*TAmainStruct_PtrPtr)->MapSizeY;
	...
	if(DataShare->TAProgress == TAInGame)
	{
		...
		for(int i=1; i<10; i++)
		{
				...
				&& DataShare->OtherMapX[i]!=-1 && DataShare->PlayerNames[i][0]!='\0')
			{
				...
				char C= PlayerDotColors[DataShare->PlayerColors[i]];
	...
	if(DataShare->TAProgress == TAInGame)
	{
		...
		for(int i=1; i<10; i++)
		{
			if(DataShare->allies[i] && DataShare->OtherMapX[i] != -1)
			{
				...
				//DestSurf->Blt(&Dest, lpMapRect, NULL, DDBLT_ASYNC | DDBLT_KEYSRCOVERRIDE, &ddbltfx);
...
void CMapRect::PaintRect()
{
	...
	//lpMapRect->Blt(NULL, NULL, NULL, DDBLT_COLORFILL | DDBLT_ASYNC, &ddbltfx);
