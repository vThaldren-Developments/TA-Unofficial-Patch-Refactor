...
{
	enum SOFTWAREDEBUGMODE : unsigned short
	{
		Drop = 1,
		Now = 2,
		Noshake = 0x10,
...
struct PlayerStruct
{
	...
	int LOS_Tileheight;
	int LOS_bitsNum;
	PlayerResourcesStruct PlayerRes; // 0x8C - Size 0x58
	float ShareMetal;
	float ShareEnergy;
	int field_EC;
	int UpdateTime;
...
}RaceSideData, *PRaceSideData; // Size - 0x222
...
struct TAdynmemStruct{
	...
	DSoundStruct *DSound;
	char data2[0x505];
	_GUIInfo desktopGUI; // 0xCCE Size
	char data3[0x97C];
	PlayerStruct Players[10];	//0x1B63 , end at 0x2851 - Size: 0xCEE
	...
	char data7[0x231];  
	POINT CurtMousePostion; // Size - 0x8 //0x2C76
	char data7_[0x10];  //
	...
	//char data9[0x6270];
	ExplosionStruct Explosions[300]; //0x1491F - Size 0x54 * 300 = 0x6270
	...
	int InterfaceType;
	char data31[0xa];//37EFE
	unsigned int gammeV;
	char data32[0x23];
	short SoftwareDebugMode;// 0x37f2f
	unsigned int  field_37F31;
	unsigned int  Senderror;
	unsigned int  RaceCounter;
	RaceSideData RaceSideDataAry[5]; // Size - 0x222 * 5 = 0xAAA
	...
	int field_39201[4]    ;//0x10 size, something about net
	int DPLAY_CONNECTION_INFO;
	int compoundaddress_;//??HAPINET_createcompoundaddress
...
typedef struct _TAProgramStruct 
{
	...
	LPBYTE GRAY_TABLE;
	LPBYTE BLUE_TABLE;
	int ScreenWidth; // 0x7C
	int ScreenHeight; // 0x80
	...
	char field_F1;
	int Max_InputBuffer;
	int InputBuffer[30]; //0x9A
	int Total_InputBuffer; //0x112
	int ReadCount_InputBuffer;
	char field_176[20];
	int MOUSE_EVENTS;
	int field_18E;
	int field_192;
	POINT CursorPos_Buf[3]; //0x136
	DWORD field_1AE;
	_GAFFrame * Cursor;
	int CursorX;
	int CursorY;
